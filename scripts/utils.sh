...
installChaincode() {
    ...
    set -x
    peer chaincode install -n $CC_NAME -v $VERSION -p $CC_SRC_PATH >&log.txt
    res=$?
    set +x
    cat log.txt
...
instantiateChaincode() {
    ...
    if [ -z $CORE_PEER_TLS_ENABLED -o $CORE_PEER_TLS_ENABLED = "false" ]; then
        set -x
    peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CC_NAME -v $VERSION -c '{"Args":[""]}' -P "OR ('ManufacturerMSP.peer','MiddleMenMSP.peer', 'ConsumerMSP.peer')">&log.txt
        res=$?
        set +x
    else
        set -x
    peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CC_NAME  -v $VERSION -c '{"Args":[""]}' -P "OR ('ManufacturerMSP.peer','MiddleMenMSP.peer', 'ConsumerMSP.peer')" >&log.txt
        res=$?
        set +x
    fi
    sleep $DELAY
